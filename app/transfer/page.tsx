...
export default function TransferPage() {
  ...
  const [loadingCubicles, setLoadingCubicles] = useState(true);
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-white via-red-50 to-red-100 font-sans">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-8 py-6 flex gap-5" style={{ height: 'calc(100vh - 73px)' }}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {loadingCubicles ? (
              <p className="text-gray-400 text-sm">Loading cubicles...</p>
            ) : services.length === 0 ? (
              <p className="text-gray-400 text-sm">
                No cubicles found. Check your Supabase table and RLS policies.
              </p>
            ) : (
              <div className="grid grid-cols-4 gap-3 content-start">
                {services.map((service) => (
                  <div
                    ...
                    onDragOver={(e) => {
                      e.preventDefault();
                      setDragOverService(service.cubicleNum);
                    }}
                    onDragLeave={() => setDragOverService(null)}
                    onDrop={() => handleDrop(service.cubicleNum)}
                    className={`bg-white border-2 rounded-3xl h-80 p-4 flex flex-col gap-2 shadow-sm transition-all duration-150 ${
