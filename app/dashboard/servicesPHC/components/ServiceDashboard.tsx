...
export function ServiceDashboard({ title, serviceFilter,}: ServiceDashboardProps) {
    ...
    <div className="px-8 py-6 flex flex-col gap-6 min-h-screen mx-auto max-w-10xl w-full">
      ...
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-4">
          <div>
            <h1 className={`text-3xl font-bold ${textDark} dark:${textLight}`}>{title}</h1>
            <div className="flex items-center gap-3 mt-2">
              {isServiceActive ? (
                ...
                <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs font-extrabold uppercase rounded-full flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-gray-400 dark:bg-gray-500"></span> Standby
                </span>
              )}
              {stats.activeRooms.length > 0 && (
                <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">
                  Rooms: <span className="text-gray-800 dark:text-gray-200">{stats.activeRooms.join(', ')}</span>
                </span>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <ServiceMetricCard className='h-40'>
          ...
          <span className="text-orange-500 dark:text-orange-400 text-5xl font-extrabold self-end mt-2">{stats.waiting}</span>
        </ServiceMetricCard>
        
        <ServiceMetricCard>
          <span className="text-gray-400 dark:text-gray-500 text-xs font-bold tracking-widest uppercase">Currently Serving</span>
          <span className="text-green-600 dark:text-green-400 text-5xl font-extrabold self-end mt-2">{stats.serving}</span>
        </ServiceMetricCard>
        
        <ServiceMetricCard>
          <span className="text-gray-400 dark:text-gray-500 text-xs font-bold tracking-widest uppercase">Done Today</span>
          <span className="text-gray-800 dark:text-gray-200 text-5xl font-extrabold self-end mt-2">{stats.served}</span>
        </ServiceMetricCard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {stats.waitingList.length > 0 ? (
              stats.waitingList.map((patient, idx) => {
                ...
                return (
                  <div 
                    key={patient.id} 
                    className={`flex items-center justify-between p-4 rounded-2xl border transition ${idx === 0 ? 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800' : 'bg-gray-50 dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:bg-gray-50/50 dark:hover:bg-gray-800/50'}`}
                  >
                    <div className="flex items-center gap-4">
                      <span className={`text-2xl font-extrabold ${idx === 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-700 dark:text-gray-300'}`}>
                        {idx + 1}. {patient.ticket}
                      </span>
                      {idx === 0 && <span className="px-2 py-1 bg-red-600 dark:bg-red-700 text-white text-xs font-bold rounded uppercase">Next</span>}
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase mb-1">Waiting For</span>
                      <span className={`text-lg font-extrabold ${isOverdue ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-200'}`}>
                        {liveMins} mins
                      </span>
                    </div>
                  </div>
              ...
              <div className="py-12 text-center text-gray-400 dark:text-gray-500 italic">No patients waiting in this queue.</div>
            )}
          </div>
        </ServiceMetricCard>
        ...
        <div className="flex flex-col gap-6">
          <ServiceMetricCard>
            ...
            <div className="flex flex-col gap-6">
              <div>
                <span className="text-gray-500 dark:text-gray-400 font-semibold block mb-1">Average Wait Time</span>
              ...
              <div>
                <span className="text-gray-500 dark:text-gray-400 font-semibold block mb-1">Longest Wait (Bottleneck)</span>
                <span className={`text-3xl font-extrabold ${isBottleneck ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-200'}`}>
                  {stats.longestWaitMins > 0 ? `${stats.longestWaitMins} min` : '--'}
                </span>
                {isBottleneck && (
                  <p className="text-xs text-red-500 dark:text-red-400 font-bold mt-2">Alert: Patient waiting over 1 hour!</p>
                )}
              </div>
            </div>
          </ServiceMetricCard>
      ...
      <ServiceMetricCard>
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Hourly patient arrivals for {title} today</p>
        </div>
